 ...
 */
public class LingjuSwipeRefreshLayout extends ViewGroup {
    ...
    private CircleImageView tCircleView;
    ...
    private MaterialProgressDrawable tProgress;
    ...
    private Animation.AnimationListener mRefreshListener = new Animation.AnimationListener() {
        ...
        public void onAnimationEnd(Animation animation) {
            ...
            if (tRefreshing) {
                ...
                if (mNotify) {
                    if (mListener != null) {
                        mListener.onDownPullRefresh();
                    }
                }
            } else {
                tProgress.stop();
                tCircleView.setVisibility(View.GONE);
                setColorViewAlpha(tCircleView, MAX_ALPHA);
                // Return the circle to its start position
                if (mScale) {
                    setAnimationProgress(tCircleView, 0 /* animation complete and view is hidden */);
                } else {
                    setTargetOffsetTopAndBottom(tCircleView, tOriginalOffsetTop - tCurrentTargetOffsetTop, true /* requires update */);
                }
            ...
            tCurrentTargetOffsetTop = tCircleView.getTop();
            bCurrentTargetOffsetTop = bCircleView.getTop();
        }
    };

    private void setColorViewAlpha(CircleImageView view, int targetAlpha) {
        if (view == tCircleView) {
            tCircleView.getBackground().setAlpha(targetAlpha);
            tProgress.setAlpha(targetAlpha);
        } else {
    ...
    public void setSize(int size) {
        ...
        // re-setting it
        tCircleView.setImageDrawable(null);
        tProgress.updateSizes(size);
        tCircleView.setImageDrawable(tProgress);
     ...
     * @param context
     */
    public LingjuSwipeRefreshLayout(Context context) {
        this(context, null);
    }
    ...
    private void createProgressView() {
        tCircleView = new CircleImageView(getContext(), CIRCLE_BG_LIGHT, CIRCLE_DIAMETER / 2);
        bCircleView = new CircleImageView(getContext(), CIRCLE_BG_LIGHT, CIRCLE_DIAMETER / 2);
        tProgress = new MaterialProgressDrawable(getContext(), this);
        bProgress = new MaterialProgressDrawable(getContext(), this);
        tProgress.setBackgroundColor(CIRCLE_BG_LIGHT);
    ...
    public void setRefreshing(boolean refreshing) {
        if (refreshing) {
            if (tRefreshing != refreshing) {
                tRefreshing = refreshing;
                int endTarget = 0;
                if (!mUsingCustomStart) {
                    endTarget = (int) (mSpinnerFinalOffset + tOriginalOffsetTop);
                } else {
                    endTarget = (int) mSpinnerFinalOffset;
                }
                setTargetOffsetTopAndBottom(tCircleView, endTarget - tCircleView.getTop(),
                        true /* requires update */);
                mNotify = false;
                startScaleUpAnimation(tCircleView, mRefreshListener);
            }
            ...
            setRefreshing(tCircleView, refreshing, false /* notify */);
    ...
    private void setRefreshing(CircleImageView view, boolean refreshing, final boolean notify) {
        if (view == tCircleView && tRefreshing != refreshing) {
            mNotify = notify;
            ensureTarget();
            tRefreshing = refreshing;
            if (tRefreshing) {
                animateOffsetToCorrectPosition(view, tCurrentTargetOffsetTop, mRefreshListener);
            } else {
                startScaleDownAnimation(view, mRefreshListener);
            }
        } else if (view == bCircleView && bRefreshing != refreshing) {
            mNotify = notify;
            ensureTarget();
            bRefreshing = refreshing;
            if (bRefreshing) {
    ...
    private Animation startAlphaAnimation(final MaterialProgressDrawable progress, final int startingAlpha, final int endingAlpha) {
        ...
        if (progress == tProgress) {
            tCircleView.setAnimationListener(null);
            tCircleView.clearAnimation();
            tCircleView.startAnimation(alpha);
        } else {
    ...
    public void setProgressBackgroundColorSchemeColor(int color) {
        tCircleView.setBackgroundColor(color);
        tProgress.setBackgroundColor(color);
    ...
    public void setColorSchemeColors(int... colors) {
        ensureTarget();
        tProgress.setColorSchemeColors(colors);
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        int circleWidth = tCircleView.getMeasuredWidth();
        int circleHeight = tCircleView.getMeasuredHeight();
        tCircleView.layout((width / 2 - circleWidth / 2), tCurrentTargetOffsetTop,
                (width / 2 + circleWidth / 2), tCurrentTargetOffsetTop + circleHeight);
        if (bOriginalOffsetCalculated) {
            circleWidth = bCircleView.getMeasuredWidth();
            circleHeight = bCircleView.getMeasuredHeight();
    ...
    public void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        tCircleView.measure(MeasureSpec.makeMeasureSpec(mCircleWidth, MeasureSpec.EXACTLY),
        ...
        if (!mUsingCustomStart && !tOriginalOffsetCalculated) {
            ...
            tCurrentTargetOffsetTop = tOriginalOffsetTop = -tCircleView.getMeasuredHeight();
        ...
        for (int index = 0; index < getChildCount(); index++) {
            if (getChildAt(index) == tCircleView) {
                tCircleViewIndex = index;
                break;
            }
        }
        bCircleViewIndex = -1;
    ...
    public int getProgressCircleDiameter() {
        return tCircleView != null ? tCircleView.getMeasuredHeight() : 0;
    }
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                if (tIsBeingDragged) {
                    ...
                    float slingshotDist = mUsingCustomStart ? mSpinnerFinalOffset
                            - tOriginalOffsetTop : mSpinnerFinalOffset;
                    ...
                    int targetY = tOriginalOffsetTop
                            + (int) ((slingshotDist * dragPercent) + extraMove);
                    // where navi.0f is a full circle
                    if (tCircleView.getVisibility() != View.VISIBLE) {
                        tCircleView.setVisibility(View.VISIBLE);
                    }
                    if (!mScale) {
                        ViewCompat.setScaleX(tCircleView, 1f);
                        ViewCompat.setScaleY(tCircleView, 1f);
                    }
                    if (overscrollTop < mTotalDragDistance) {
                        if (mScale) {
                            setAnimationProgress(tCircleView, overscrollTop / mTotalDragDistance);
                        }
                        if (tProgress.getAlpha() > STARTING_PROGRESS_ALPHA
                                && !isAnimationRunning(mAlphaStartAnimation)) {
                            // Animate the alpha
                            startProgressAlphaStartAnimation(tProgress);
                        }
                        float strokeStart = adjustedPercent * .8f;
                        tProgress.setStartEndTrim(0f, Math.min(MAX_PROGRESS_ANGLE, strokeStart));
                        tProgress.setArrowScale(Math.min(1f, adjustedPercent));
                    } else {
                        if (tProgress.getAlpha() < MAX_ALPHA
                                && !isAnimationRunning(mAlphaMaxAnimation)) {
                            // Animate the alpha
                            startProgressAlphaMaxAnimation(tProgress);
                        }
                    }
                    float rotation = (-0.25f + .4f * adjustedPercent + tensionPercent * 2) * .5f;
                    tProgress.setProgressRotation(rotation);
                    setTargetOffsetTopAndBottom(tCircleView, targetY - tCurrentTargetOffsetTop, true);
            ...
            case MotionEvent.ACTION_CANCEL: {
                ...
                if (Math.abs(overscrollTop) > mTotalDragDistance) {
                    setRefreshing(tIsBeingDragged ? tCircleView : bCircleView, true, true /* notify */);
                } else {
                    // cancel refresh
                    tRefreshing = false;
                    ...
                    if (!mScale) {
                        listener = new Animation.AnimationListener() {
                            ...
                            public void onAnimationEnd(Animation animation) {
                                if (!mScale) {
                                    startScaleDownAnimation(!canChildScrollUp() ? tCircleView : bCircleView, null);
                                }
                            }
                    ...
                    animateOffsetToStartPosition(tIsBeingDragged ? tCircleView : bCircleView,
                            tIsBeingDragged ? tCurrentTargetOffsetTop : bCurrentTargetOffsetTop, listener);
    ...
    private final Animation mAnimateToCorrectPosition = new Animation() {
        ...
        public void applyTransformation(float interpolatedTime, Transformation t) {
            ...
            if (tRefreshing) {
                if (!mUsingCustomStart) {
                    endTarget = (int) (mSpinnerFinalOffset - Math.abs(tOriginalOffsetTop));
                } else {
                    endTarget = (int) mSpinnerFinalOffset;
                }
                targetTop = (mFrom + (int) ((endTarget - mFrom) * interpolatedTime));
                int offset = targetTop - tCircleView.getTop();
                setTargetOffsetTopAndBottom(tCircleView, offset, false /* requires update */);
                tProgress.setArrowScale(1 - interpolatedTime);
            } else if (bRefreshing) {
                endTarget = bOriginalOffsetTop - (int) mSpinnerFinalOffset;
                targetTop = (mFrom + (int) ((endTarget - mFrom) * interpolatedTime));
    ...
    private void moveToStart(float interpolatedTime) {
        int targetTop = 0;
        boolean isTop = !canChildScrollUp();
        if (isTop) {
            targetTop = (mFrom + (int) ((tOriginalOffsetTop - mFrom) * interpolatedTime));
            int offset = targetTop - tCircleView.getTop();
            setTargetOffsetTopAndBottom(tCircleView, offset, false /* requires update */);
        } else {
